...
;
; RUN: llvm-as < %s > %t1
; RUN: llvm-lto -O0 -struct-field-cache-analysis -struct-analysis-FRG-only -struct-analysis-disable-ignore-zeros -debug-only=struct-analysis -o %t2 %t1 2>&1 | FileCheck %s
